...
function showBusinessCard() {
  ...
  document.querySelector('.business-card.header-text').innerHTML =
    (jsonData.prefix ? jsonData.prefix + ' ' : '') + jsonData.full_name;
}
      ...
      registration.pushManager
        .subscribe({
          ...
          applicationServerKey: urlB64ToUint8Array(
            'BBJJzPB7_LO0a5z_LVn5wZgOH7s5S-r_1RXMsR8-rQjJiTWsiPv1Iynacc3qAexps5Dm8LpjdTH5NV2gA929je0'
...
// Function to detect device type, model, and add class to body
function detectDeviceAndAddClass() {
  ...
  if (/android/i.test(userAgent)) {
    deviceClass = 'android-device';
    const match = userAgent.match(/; ([^;]+) build\//i);
    if (match) modelClass = `android-${match[1].replace(/\s+/g, '-').toLowerCase()}`;
    ...
    if (/ipad/i.test(userAgent)) {
      ...
      const match = userAgent.match(
        /iphone\s*(?:3gs|4|4s|5|5c|5s|6|6 plus|6s|6s plus|se|7|7 plus|8|8 plus|x|xs|xs max|xr|11|11 pro|11 pro max|12|12 mini|12 pro|12 pro max|13|13 mini|13 pro|13 pro max|14|14 plus|14 pro|14 pro max)/i
      );
      if (match) modelClass = `iphone-${match[1].replace(/\s+/g, '-').toLowerCase()}`;
    }
  } else if (/windows phone/i.test(userAgent)) {
    deviceClass = 'windows-phone-device';
    const match = userAgent.match(/windows phone (?:os)?\s?([^;]+)/i);
    if (match) modelClass = `wp-${match[1].replace(/\s+/g, '-').toLowerCase()}`;
  } else if (/macintosh|mac os x/i.test(userAgent)) {
    deviceClass = 'mac-device';
    modelClass = 'mac';
  } else if (/windows|win32/i.test(userAgent)) {
    deviceClass = 'windows-device';
    modelClass = 'windows';
  } else {
    deviceClass = 'unknown-device';
  }

  // Check for curved corner screen
  let hasCurvedScreen = false;

  // iOS devices with notch or dynamic island (curved corners)
  if (/iphone/i.test(userAgent)) {
    ...
    for (let model of notchModels) {
      if (userAgent.includes(model)) {
        hasCurvedScreen = true;
        break;
      }
    }
  }

  // Android devices (approximate check based on release date)
  // if (/android/i.test(userAgent)) {
  //     const androidVersion = parseFloat(userAgent.match(/android\s([0-9.]*)/i)?.[1] || '0');
  //     if (androidVersion >= 9) { // Android Pie (9.0) and above often have curved screens
  //         hasCurvedScreen = true;
  //     }
  // }
  ...
  if (hasCurvedScreen) {
    document.body.classList.add('curved-screen');
  }
  if (modelClass) document.body.classList.add(modelClass);
}
